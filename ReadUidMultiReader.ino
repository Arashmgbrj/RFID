...
void dump_byte_array(byte *buffer, byte bufferSize) {
  for (byte i = 0; i < bufferSize; i++) {
   // Serial.print(buffer[i] < 0x10 ? " 0" : " ");
    ...
    //x1=buffer[i];
//   st[i]=buffer[i];
 // sprintf(st,"%d%d%d%d%d%d%d%d%d%d",buffer[0],buffer[1],buffer[2],buffer[3],buffer[4],buffer[5],buffer[6],buffer[7],buffer[8],buffer[9]);
    //Serial.print(st[i]);
     st[i]=buffer[i]; 
     
  }
if  ((st[0]==240)){Serial.println("ok");}else{Serial.println("deez");}
Serial.print(st[0]);
